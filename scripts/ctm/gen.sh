...
case "$method" in
    "full")
        TILE_LAST_INDEX=46
        . "$script_dir/gen_full.sh"
    ;;

    "compact")
        TILE_LAST_INDEX=4
        . "$script_dir/gen_compact.sh"
    ;;
...
echo "Generating CTM with method '$method' and tile size $size."
echo "Mod ID: '$mod_id'"
...
if [[ -f "$animation_mcmeta_file" && -f "$animation_frames_file" ]]; then
    ...
    echo "Animation detected! The generated CTM will be animated with a frame count of $animation_frames."
    ...
    echo "Generating animated CTM images..."
